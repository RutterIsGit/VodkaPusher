...
def parse_fhrs(xml_bytes):
    ...
    for est in root.find_all("EstablishmentDetail"):
        ...
        yield {
            ...
            "address_line2": est.AddressLine4.string if est.AddressLine4 else "",
...
if opubs_content:
    with zipfile.ZipFile(io.BytesIO(opubs_content)) as z:
        with z.open(z.namelist()[0]) as f:
            ...
            next(rdr)  # Skip the first row, which is actually the header row in the data
...
with OUT.open("w", newline="", encoding="utf-8") as f:
    ...
    w.writeheader(); w.writerows(deduped_rows)